...
function calculateMedian() {
    ...
    reader.onload = function(event) {
        ...
        if (count === 0) {
            medianResult.textContent = 'The file is empty or contains no valid numbers.';
            verboseOutput.style.display = 'none';
            return;
        }
        ...
        if (verboseFlag) {
            ...
            verboseOutput.innerHTML = `<p>Sorted numbers: ${sortedNumbers.join(', ')}</p><p>Total count: ${count}</p>`;
        } else {
            verboseOutput.style.display = 'none';
        }
    };
    ...
    reader.onerror = function() {
        medianResult.textContent = 'Error reading file.';
        verboseOutput.style.display = 'none';
    };
